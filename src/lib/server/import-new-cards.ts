...
// running it doesn't affect today's snapshot or the collection graphs.
...
async function importNewCards(filePath: string) {
	...
	console.log('Streaming bulk file, inserting only new cards...');
	for await (const card of parseScryfallBulkStream<ScryfallCard>(filePath)) {
		pendingBatch.push(card);
		scanned++;
		if (pendingBatch.length >= batchSize) {
			applyBatch();
			process.stdout.write(`\rScanned ${scanned}, inserted ${inserted}...`);
		}
	}
	if (pendingBatch.length > 0) applyBatch();

	console.log(`\nDone. Scanned ${scanned} cards, inserted ${inserted} new cards.`);
	sqlite.close();
}
