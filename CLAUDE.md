...
Status: WP3.1 and WP3.2 done (2026-09-16), measured with the hashes of 20 sets while the full run continues on the dev PC. Eight photos against the full catalogue: identity 95 → **103 of 104**, printing 70 → **79**, 1 `likely`, **0 wrong** (acceptance: identity ≥ 100 with 0 wrong — met); seeded DB 103 / 103 without hashes, **104 / 104** with the hashes imported; hold-out identity 16 → 18 of 26 (the two flavor-name cards are recognised by their artwork), printing 5 of 26 unchanged, 0 wrong. Open: the full table (~115k hashes) still has to be measured for false matches at ≤ 10 bits before the export goes to the host; WP3.3 only if that measurement demands it.
...
- Acceptance: identity ≥ 100/104 on the eight photos with 0 wrong identities; the hold-out set reported separately; printing rules unchanged. Measured 2026-09-16 (hashes of 20 sets): 103 / 104, 0 wrong; hold-out 18 / 26, 0 wrong; the printing rules are unchanged apart from the narrowing above.
...
| **Phase 3, full catalogue (measured, art hashes of 20 sets)** | **103** | **79** (24 open, footer still needed for same-art reprints) | **0** |
...
- **Art-hash coverage.** A printing is recognised (or excluded) by its artwork only when its hash exists; the full run (~6 h once on the dev PC, `npm run hash-art`) is still in progress and the host receives the table via `--export` / `--import` afterwards. Until then the visual channel covers 20 sets, and hold-out cards such as Koma (8 printings, 6 without a hash) keep their printing open although the artwork identified them. The false-match rate of the *full* table at ≤ 10 bits is not measured yet — do that before the import.
...
- **Hold-out classes (2026-09-16, not tuned on):** Universes Beyond *flavor names* ("The Monstrous Serpent" for Koma, Cosmos Serpent; "Wild Rose Rebellion" for Counterspell) are unknown to the catalogue — the import needs Scryfall's `flavor_name` and the search a further alias; showcase footers (SOA, TLE) carry the collector number on the line *above* "SET • EN", outside the strip crop, so those printings stay open; a black-bordered card filling the frame on a dark cloth makes every detection strategy return the image frame, in single and multiple mode; old frames (Alliances, a numberless Tropical Island) defeat the name passes and have no collector line.
